...
#include "r_bin_dietelf_static.h"
...
int ELF_(dietelf_close)(ELF_(dietelf_bin_t) *bin) {
	close(bin->fd);
...
u64
ELF_(dietelf_get_section_index)(ELF_(dietelf_bin_t) *bin, const char *section_name)
{
	ELF_(Ehdr) *ehdr = &bin->ehdr;
	ELF_(Shdr) *shdr = bin->shdr, *shdrp;
...
u64 ELF_(dietelf_get_section_offset)(ELF_(dietelf_bin_t) *bin, const char *section_name)
{
	ELF_(Ehdr) *ehdr = &bin->ehdr;
	ELF_(Shdr) *shdr = bin->shdr, *shdrp;
	const char *string = bin->string;
...
int ELF_(dietelf_get_section_size)(ELF_(dietelf_bin_t) *bin, const char *section_name)
{
	ELF_(Ehdr) *ehdr = &bin->ehdr;
	ELF_(Shdr) *shdr = bin->shdr, *shdrp;
	const char *string = bin->string;
...
static u64 ELF_(get_import_addr)(ELF_(dietelf_bin_t) *bin, int sym)
{
	int fd = bin->fd;
	ELF_(Ehdr) *ehdr = &bin->ehdr;
	ELF_(Shdr) *shdr = bin->shdr, *shdrp;
...
int ELF_(dietelf_get_sections)(ELF_(dietelf_bin_t) *bin, dietelf_section *section)
{
	ELF_(Ehdr) *ehdr = &bin->ehdr;
	ELF_(Shdr) *shdr = bin->shdr, *shdrp;
...
int ELF_(dietelf_get_imports)(ELF_(dietelf_bin_t) *bin, dietelf_import *import)
{
	int fd = bin->fd;
	ELF_(Ehdr) *ehdr = &bin->ehdr;
	ELF_(Shdr) *shdr = bin->shdr, *shdrp;
...
int ELF_(dietelf_get_imports_count)(ELF_(dietelf_bin_t) *bin)
{
	int fd = bin->fd;
	ELF_(Ehdr) *ehdr = &bin->ehdr;
	ELF_(Shdr) *shdr = bin->shdr, *shdrp;
...
int ELF_(dietelf_get_symbols)(ELF_(dietelf_bin_t) *bin, dietelf_symbol *symbol)
{
	int fd = bin->fd;
	ELF_(Ehdr) *ehdr = &bin->ehdr;
	ELF_(Shdr) *shdr = bin->shdr, *shdrp;
	...
	sym_offset = (bin->ehdr.e_type == ET_REL ? ELF_(dietelf_get_section_offset)(bin, ".text") : 0);
...
int ELF_(dietelf_get_symbols_count)(ELF_(dietelf_bin_t) *bin)
{
	int fd = bin->fd;
	ELF_(Ehdr) *ehdr = &bin->ehdr;
	ELF_(Shdr) *shdr = bin->shdr, *shdrp;
...
static int ELF_(dietelf_init)(ELF_(dietelf_bin_t) *bin)
{
	int fd = bin->fd;
	ELF_(Ehdr) *ehdr;
	ELF_(Shdr) *shdr;
